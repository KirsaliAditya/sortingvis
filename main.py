...
def draw(draw_info, algo_name, ascending):
    ...
    sorting = draw_info.FONT.render("I - Insertion Sort | B - Bubble Sort | S - Selection Sort", 1,
                                    draw_info.BLACK)
...
def selection_sort(draw_info, ascending=True):
    ...
    for i in range(len(lst)):
        ...
        for j in range(i + 1, len(lst)):
            if (ascending and lst[j] < lst[current]) or (not ascending and lst[j] > lst[current]):
                current = j

        if i != current:
            lst[i], lst[current] = lst[current], lst[i]
            draw_list(draw_info, {i: draw_info.GREEN, current: draw_info.RED}, True)
            yield True
